...
static esp_err_t mqtt_event_handler(esp_mqtt_event_handle_t event)
{
  switch (event->event_id)
  {
    ...
    case MQTT_EVENT_DATA:
      ...
      incoming_data[i] = '\0';
      Logger.Info("Data: " + String(incoming_data));  ///  THIS is the capture of incoming message from cloud to device send.
...
void loop()
{
  ...
  else if (millis() > next_telemetry_send_time_ms)
  {
    ...
    // sendTelemetry();  commented to prevent send to cloud.
